...
void MainWindow::on_button_equal_released()
{
    ...
    //Prepisovani klasickeho symbolu odmocniny na format root{exponent,mocnenec}
    int len = ui->result->text().length();
    QString expression =  ui->result->text();

    for(int i = 0; i < len; i++){
        if(expression[i] == QString::fromUtf8("\u221A")){
            expression.insert((i-1), "root{");
            expression.replace((i+5), 1, ",");
            expression.insert((i+7), "}");
